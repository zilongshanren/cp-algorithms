...
int main() {
    ...
    //r denote the dan's position in the fence
	for (int i = 0; i < r; i++) {
		int m;
		cin >> m;
       ...
       // 首先我们要明白一个定理：向量a×向量b（×为向量叉乘），若结果小于0，表示向量b在向量a的顺时针方向；
        //若结果大于0，表示向量b在向量a的逆时针方向；若等于0，表示向量a与向量b平行。

        //排序之后，所有的边相对于origin 是逆时针方向排序的
		sort(polygon, polygon + m, [](pair<ll, ll> a, pair<ll, ll> b) { return cross(a, b) > 0; });

		events.push_back({1, (short)i, polygon[0]});
		events.push_back({-1, (short)i, polygon[m - 1]});
	}
